...
class Admin extends CI_Controller
{
    ...
    public function entr()
    {
		...
		$this->template->load('template', 'entr_baru', $data);
    }
    ...
    public function vedit($id)
    {
        if ($id != "") {
            ...
            if (count($row) > 0) {
                $this->template->load('template', 'edit1', $row);
    ...
    public function klas()
    {
        ...
        $this->template->load('template', 'klas', $data);
    }
    ...
    public function penc()
    {
        ...
        $this->template->load('template', 'pencipta', $data);
    }
    ...
    public function adduser()
    {
        ...
        $q = sprintf("INSERT INTO master_user (username,password,tipe,akses_klas,akses_modul,email) VALUES ('%s', '%s',%s,'%s','%s','%s')",
            $username, $password, $tipe, $akses_klas, $akses_modul,$email);
        $hsl = $this->db->query($q);
        if ($hsl) {
