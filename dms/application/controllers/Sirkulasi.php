...
class Sirkulasi extends CI_Controller {
    ...
    protected function src()
    {
		...
		$sql = "SELECT s.*, u.username, a.uraian,
        ...
        // die($sql);
        // row count
		$sql_row = "SELECT COUNT(*) AS total FROM sirkulasi AS s JOIN data_arsip AS a ON a.noarsip=s.noarsip JOIN master_user AS u ON s.username_peminjam=u.username";
        // die($sql);
	...
	public function datalist($offset=0)
	{
		...
		//echo cekarray2($data);
		//echo cekdb();
		//$this->session->set_flashdata('zz', $data);
		...
		$this->template->set('title', 'Sirkulasi');
		$this->template->load('template', 'sirkulasi/main', $data);
    ...
    $this->template->set('title', 'Sirkulasi');
		$this->template->load('template', 'sirkulasi/entri', $data);
	}
	...
	public function vedit($id)
	{
		if($id!=""){
			...
			if(count($row)>0) {
				...
				$this->template->set('title', 'Sirkulasi');
				$this->template->load('template', 'sirkulasi/edit', $row);
			}else{
				redirect('/sirkulasi/', 'refresh');
			}
		}else {
	...
	public function search_arsip(){
        ...
        $this->db->like('noarsip', $term);
 
        $data = $this->db->get("data_arsip")->result();
