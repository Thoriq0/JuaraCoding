...
$(document).ready(function() {
	...
	var site_url = "https://dev.ptdika.com/dokumen/" ;
	...
	function reloadpenc() {
		$.ajax({
			...
			// url: site_url + "/admin/reloaddepartemen",
			cache: false,
			success: function(html) {
	...
	function delpenc() {
		alert("Data departemensdsda telah sukses dihapus ");
		$("#delpenc").modal("hide");
		reloadpenc();
	}
	...
	function edpenc() {
		alert("Data pencipta telah sukses disimpan");
		$("#editpenc").modal("hide");
		reloadpenc();
	}
	...
	function addpeng() {
		alert("Data telah suksess disimpan");
		location.reload();
		$("#addpeng").modal("hide");
		$("#faddpeng")[0].reset();
		reloadpeng();
	...
	// Fungsi internal dokumen

	function reloadintdoc() {
		$.ajax({
			type: "POST",
			url: site_url + "/admin/list_internal_dokumen",
			cache: false,
			success: function(html) {
				$("#divtabeldoc").html(html);
			}
		});
	}
	...
	$("#fdeldatadoc").ajaxForm({ success: deldata });
	function deldata() {
		alert("Data internal dokumen telah sukses dihapus 123");
		$("#deldata").modal("hide");
		// alert("oke brooo");
	...
	// });
	/** MASTER TEMPLATE */
	$("#divtabeltemp").on("click", ".deltemp", function() {
		var d = $(this).attr("id");
		$("#delidtemp").val(d);
	});

	$("#deltempgo").on("click", function() {
		$("#fdeltemp").submit();
	});
	$("#fdeltemp").ajaxForm({ success: deltemp });
	function deltemp() {
		alert("Data telah sukses dihapus");
		// reloaduser();
		$("#deltemp").modal("hide");
